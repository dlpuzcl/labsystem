                            ...
                            <div class=" col-md-12">
                                <div class="table-responsive">
                                    ...
                                    <table width="100%" class="table table-striped table-bordered table-hover"
                                        ...
                                        <tbody>
                                        <c:forEach items="${profession}" var="row" varStatus="vs">
                                            <tr>
                                                <td>${vs.count }</td>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="ModalLabel">添加专业</h4>
                    </div>
                    <div class="modal-body">
                        <form class="form-horizontal" id="add_Profession_form">
                            <input type="hidden" id="dict_type_code" name="dict_type_code" value="004"/>
                            <input type="hidden" id="dict_type_name" name="dict_type_name" value="专业"/>

                            <div class="form-group">
                                <label for="dict_item_name" class="col-sm-2 control-label">专业</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="dict_item_name" placeholder="专业"
                                           name="dict_item_name">
                                </div>
                            </div>
        ...
        $("#add_Profession_form").validate({
            ...
            submitHandler: function (form) {  //表单提交后要执行的内容
                $.post("<%=basePath%>baseDict/add.action", $("#add_Profession_form").serialize(), function (data) {
                    if (data == "0") {
                        swal({title: "成功", text: "专业添加成功", type: "success"}, function () {
                            window.location.reload();
                        });

                    }else {
                        swal("失败！", "专业添加失败。", "error");
                    }
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.post("<%=basePath%>baseDict/delete.action", {"id": id}, function (data) {
                            if (data == "0") {
                                swal({title: "删除！", text: "专业已经被删除。", type: "success"},
                                    function () {
                                        window.location.reload();
                                    });
